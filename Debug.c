...
/**                                                         **/
/** File Adapted by J. BRIGAUD to treat other user commands **/
/*************************************************************/
...
/*----------+-----------+--------+------------------------------------------*/
/* 16/02/04 | J.BRIGAUD |  0003  | Add k7 command to load a k7              */
/*          |           |        |     go command                           */
/*          |           |        |                                          */
/*----------+-----------+--------+------------------------------------------*/
...
/** DisplayZ80Register() *************************************/
/** DisplayZ80Register() will display the Z80 Register      **/
/*************************************************************/
static void DisplayZ80Register (Z80 *R)
{
 const char Flags[8] = "SZ.H.PNC";
 char  Str_F_Reg[9];
 char  StrDisasemble [20];
 byte J,I;

 DAsm(StrDisasemble,R->PC.W);
 for (J=0,I=R->AF.B.l;J<8;J++,I<<=1)
  Str_F_Reg[J]=I&0x80? Flags[J]:'.';
 Str_F_Reg[8]='\0';
 ...
 fprintf
  ...
  fprintf (stderr,"AF':%04X HL':%04X DE':%04X BC':%04X IFF:%02X I:%02X\n",
           R->AF1.W,R->HL1.W,R->DE1.W,R->BC1.W,R->IFF,R->I);
...
/*  printf
  ...
  fprintf (stderr,"%04X: %-15.15s   AT SP: [%04X]   FLAGS: [%s]\n\n",
           R->PC.W,StrDisasemble,RdZ80(R->SP.W)+RdZ80(R->SP.W+1)*256,Str_F_Reg );  
}
/** DebugZ80() ***********************************************/
...
byte DebugZ80(Z80 *R)
{
  char UserCmd_O[128];       // Origin command user
  char UserCmd[128];         // Upper command user
  byte J,I;                  // Loop index
  byte EndLoop=0;            // Flag To exit loop
  byte Result = NOT_END_PGM; // Exit result 0 = End of PGM

  /* Display the Z80 Register */
  /*--------------------------*/
  DisplayZ80Register (R);

  /* Treat the User commands */
  /*-------------------------*/
  while(!EndLoop)
  {
    printf("\n[Command,'?']-> ");
    ...
    switch(UserCmd[0])
    {
      ...
      case '?':
        ...
        puts ("r               : Display Registers");
        puts ("s <reg> <value> : Set a register with a value");
        puts ("?,h             : Show this help text");
        puts ("q               : Exit Z80 emulation");
	puts ("k7 <file>       : Load a k7 file");
        puts ("go              : Continue without break");
        break;

      case '\0': EndLoop=1;
		 break;

      case '=':  if(strlen(UserCmd)>=2)
                  { 
		   sscanf(UserCmd+1,"%hX",&(R->Trap));
		   R->Trace=0;
		   EndLoop=1;
		 }
                 break;
      ...
      case 'J':  if(strlen(UserCmd)>=2)
                  { 
		   sscanf(UserCmd+1,"%hX",&(R->PC.W));
		   R->Trace=0;
		   EndLoop=1;
		  }
                 break;
      ...
      case 'M':
        {
          ...
          if(strlen(UserCmd)>1) sscanf(UserCmd+1,"%hX",&Addr); else Addr=R->PC.W;
          puts("");
          for(J=0;J<16;J++)
          {
            fprintf (stderr,"%04X: ",Addr);
            for(I=0;I<16;I++,Addr++)
              fprintf (stderr,"%02X ",RdZ80(Addr));
            fprintf (stderr," | ");Addr-=16;
            for(I=0;I<16;I++,Addr++)
 ...
 fprintf (stderr,"Chaine saisie: %s \n",UserCmd);
 
          if(strlen(UserCmd)>1)
           {
            sscanf(UserCmd+1,"%s %hX",Reg,&Value);
            if (!strcmp(Reg,"AF")) R->AF.W   = Value;
            if (!strcmp(Reg,"A"))  R->AF.B.h = (byte) (Value & 0xFF);
            if (!strcmp(Reg,"F"))  R->AF.B.l = (byte) (Value & 0xFF);
            if (!strcmp(Reg,"BC")) R->BC.W   = Value;
            if (!strcmp(Reg,"B"))  R->BC.B.h = (byte) (Value & 0xFF);
            if (!strcmp(Reg,"C"))  R->BC.B.l = (byte) (Value & 0xFF);
            if (!strcmp(Reg,"DE")) R->DE.W   = Value;
            if (!strcmp(Reg,"D"))  R->DE.B.h = (byte) (Value & 0xFF);
            if (!strcmp(Reg,"E"))  R->DE.B.l = (byte) (Value & 0xFF);
            if (!strcmp(Reg,"HL")) R->HL.W   = Value;
            if (!strcmp(Reg,"H"))  R->HL.B.h = (byte) (Value & 0xFF);
            if (!strcmp(Reg,"L"))  R->HL.B.l = (byte) (Value & 0xFF);
            
            if (!strcmp(Reg,"PC")) R->PC.W = Value;
           }
        }
        break;
	 ...
	 if (*(UserCmd+1) == '7')
	  {
	   if (*(UserCmd+2))
	    {
	     char Buffer[80];
	     sscanf (UserCmd_O+2,"%s",Buffer);
	     fprintf (stderr,"Read k7 : %s\n",Buffer);
	     open_k7 (Buffer);
	    }
	   else
	    {
